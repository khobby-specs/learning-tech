...
- Like the name says,  Hypertext **Transfer Protocol** secure, it is a protocol that protect data in transit and not data at its destination. Immediately data gets to the server, it is no longer protected by HTTPS. The data gets decrypted and stored somewhere in the server's database. When the server gets compromised or attacked, all your data in its database is accessible to to the attacker. This is is not a sign that HTTPS failed. It simply tell us that HTTPS secures not the destination, but rather the server. 
