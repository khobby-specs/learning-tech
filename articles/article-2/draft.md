...
- Like the name says,  Hypertext **Transfer Protocol** secure, it is a protocole that protolect data in transit. Immediately data gets to ther server, it is no longer protected by HTTPS. The data get ls decrypted and store somewhere in the server's database. When the server get compromised or attack, all your data in it's database is clear accessible to to the attacker. This is is not a sign that HTTPS failed. It simply tell us that HTTPS secures not the destination, but rather the server. 
